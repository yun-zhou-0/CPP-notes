...
public:
  ...
  // key_compare 和 value_compare 是同一个函数
public:
...
private:
  ...
  定义
  template <class T>
  struct identity : publiuc unary_function<T, T>{
...
public:
  ...
  // 将iterator 定义为rb_tree的const_iterator,表明set的迭代器无法写入
  ...
  // accessors:
  // set的操作行为，都使用rbtree的操作，此处只需要调用即可
